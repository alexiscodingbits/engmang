  ...
  return (
    <main className="min-h-screen bg-zinc-950 pt-20 pb-12">
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 mb-8">
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              {profileUser.bio && (
                <p className="text-zinc-400 text-sm mt-2 leading-relaxed">{profileUser.bio}</p>
              )}
              ...
              <div className="flex items-center gap-3 mt-4 flex-wrap">
                ...
                {isOwnProfile && (
                  <Link
                    href="/profile/edit"
                    className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-zinc-700 hover:bg-zinc-600 text-zinc-200 transition-colors"
                  >
                    Edit Profile
                  </Link>
                )}
        ...
        {posts.length === 0 ? (
          <p className="text-zinc-600 text-sm">No public posts yet.</p>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      {post.noteTag ? (
                        <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-teal-900 text-teal-300 mb-2 inline-block">
                          {post.noteTag}
                        </span>
                      ) : (
                      ...
                      <p className="text-zinc-200 text-sm font-medium leading-snug">{post.title}</p>
                      {post.body && (
                        <p className="text-zinc-500 text-xs mt-1 line-clamp-2">{post.body}</p>
                      )}
                    </div>
                  </div>
